...
function reemplazar(){
    ...
    let outtxtv = outtxt.innerText = intxtv.replaceAll(tofindv,toreplacev);

    let coincidencias = exampletxt.match(intxtv);

    let cantidad = coincidencias ? coincidencias.length : 0;


    outtxt.innerText = outtxtv + `Se realizaron ${cantidad} reemplazos.` ;
